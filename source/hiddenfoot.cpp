// =========================================================================
// HiddenFoot V0.9 (January 2023)
// CREATED BY NACHO MOLINA
// =========================================================================
// Compiling
// clang++ -O3 -Xpreprocessor -fopenmp -L/opt/local/lib/libomp/ -I/opt/local/include/libomp/  hiddenfoot.cpp -o hiddenfoot -lomp
// g++-mp-12 -O3 -Xpreprocessor -fopenmp -L/opt/local/lib/libomp/ -I/opt/local/include/libomp/  hiddenfoot.cpp -o hiddenfoot -lomp
//
// Things to do to finish version 1.0
// - Sanity checks
// - Include Pol-II
// - Option of less/sparse printing
// - Priting sumary statsitcs of parmters

// Things to do to finish version 2.0-
// - Adaptative MCMC
// - SG-MCMC
// - Analyze multipe regions at once
// - Parallelization (OpenMP)
// - Sparse matrices for methylation 
// - Improve reading of files 
// - Using vectors (?)
...
int main(int argc, char** argv) {
  ...
  // Reading arguments
  parse_argv(argc,argv,&files,&runmod,&parwms,&parsgd,&parsam,&parsim);
  ...
  // Simulatng binding and methylation profiiles
  if (runmod.sim == 1) {get_simulations(seq,met,wms,params,parsim,files.out);}
...
void parse_argv(int argc, char **argv, typefiles *files, typerunmod *runmod, typeparwms *parwms, typeparsgd *parsgd, typeparsam *parsam, typeparsim *parsim) {
  ...
  if      (mode == "run") {runmod -> sgd  = 1; runmod -> sam  = 1; runmod -> bnd  = 1; runmod -> sim  = 0;}
  else if (mode == "fit") {runmod -> sgd  = 1; runmod -> sam  = 1; runmod -> bnd  = 0; runmod -> sim  = 0;}
  else if (mode == "bnd") {runmod -> sgd  = 0; runmod -> sam  = 0; runmod -> bnd  = 1; runmod -> sim  = 0;}
  else if (mode == "sim") {runmod -> sgd  = 0; runmod -> sam  = 0; runmod -> bnd  = 0; runmod -> sim  = 1;}
  else                    {errorout();}
